...
class ModelModel {
  ...
  static async findById(id) {
    try {
      const [rows] = await pool.execute(
        'SELECT m.id, m.modelo, ma.marca AS marca, ma.id AS id_marca FROM modelo m JOIN marca ma ON m.id_marca = ma.id WHERE m.id = ?',
        [id]
      );
      return rows[0];
    } catch (error) {
  ...
  static async delete(id) {
    try {
      const [result] = await pool.execute('DELETE FROM modelo WHERE id = ?', [id]);
      return result.affectedRows > 0;
    } catch (error) {
